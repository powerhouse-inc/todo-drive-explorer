    ...
    (state, action, dispatch) => {
        ...
        switch (action.type) {
            case "ADD_TODO_ITEM":
                ...
                ToDoListReducer.addTodoItemOperation(state[action.scope], action, dispatch);
                break;

            case "UPDATE_TODO_ITEM":
                z.UpdateTodoItemInputSchema().parse(action.input);
                ToDoListReducer.updateTodoItemOperation(state[action.scope], action, dispatch);
                break;

            case "DELETE_TODO_ITEM":
                z.DeleteTodoItemInputSchema().parse(action.input);
                ToDoListReducer.deleteTodoItemOperation(state[action.scope], action, dispatch);
                break;
